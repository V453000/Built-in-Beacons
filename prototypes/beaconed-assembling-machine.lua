...
beaconed_assembling_machine.energy_source.emissions_per_minute.pollution = beaconed_stats(beaconed_data).beaconed_emissions_per_minute
...
if settings.startup["productivity-mode"].value == 'Modded Modules' then
  beaconed_assembling_machine.module_slots = original_assembling_machine_3.module_slots * 2
  if settings.startup["show-module-slot-row-length"].value > 0 then
    beaconed_assembling_machine.module_info_max_icons_per_row = settings.startup["show-module-slot-row-length"].value
  end
  if settings.startup["show-module-slot-rows"].value > 0 then
    beaconed_assembling_machine.module_info_max_icon_rows = settings.startup["show-module-slot-rows"].value
  end
else
  beaconed_assembling_machine.module_slots = 0
  beaconed_assembling_machine.base_productivity = beaconed_stats(beaconed_data).beaconed_base_productivity
end

if settings.startup["modded-entity-graphics"].value == "ON" then
  beaconed_assembling_machine.fluid_boxes[1].secondary_draw_order = 2
  beaconed_assembling_machine.fluid_boxes[2].secondary_draw_order = 2
  table.insert(beaconed_assembling_machine.graphics_set.animation.layers, 
    {
...
if settings.startup["modded-entity-graphics"].value ~= "OFF" then
  for i,layer in pairs(beaconed_assembling_machine.graphics_set.animation.layers) do
...
data:extend({
  {
    ...
    stack_size = 50,
    localised_description = {'item-description.beaconed-assembling-machine', tostring(global_assembling_machine_beacon_count)}
  },
})
...
data:extend({
  {
    ...
    ingredients =
    {
      ...
      {type = "item", name = "speed-module", amount = global_assembling_machine_average_beacon_count * global_beacon_module_slots}
    },
    results = {
