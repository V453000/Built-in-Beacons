...
beaconed_chemical_plant_2.energy_source.emissions_per_minute.pollution = beaconed_stats(beaconed_data).beaconed_emissions_per_minute--4 / 210000
...
if settings.startup["productivity-mode"].value == 'Modded Modules' then
  beaconed_chemical_plant_2.module_slots = data.raw["assembling-machine"]["chemical-plant"].module_slots * 2
  if settings.startup["show-module-slot-row-length"].value > 0 then
    beaconed_chemical_plant_2.module_info_max_icons_per_row = settings.startup["show-module-slot-row-length"].value
  end
  if settings.startup["show-module-slot-rows"].value > 0 then
    beaconed_chemical_plant_2.module_info_max_icon_rows = settings.startup["show-module-slot-rows"].value
  end
else
  beaconed_chemical_plant_2.module_slots = 0
  beaconed_chemical_plant_2.base_productivity = beaconed_stats(beaconed_data).beaconed_base_productivity
end
...
if settings.startup["modded-entity-graphics"].value == "ON" then
  ...
  merge_layers(beaconed_chemical_plant_2.graphics_set.animation, make_4way_animation_from_spritesheet(
    {
      layers =
      {
        {
...
if settings.startup["modded-entity-graphics"].value ~= "OFF" then
  ...
  for i,layer in pairs(beaconed_chemical_plant_2.graphics_set.working_visualisations) do
...
data:extend({
  {
    ...
    stack_size = 10,
    localised_description = {'item-description.beaconed-chemical-plant-2', tostring(global_chemical_plant_beacon_count)}
  },
})
data:extend({
  {
    ...
    ingredients =
    {
      ...
      {type = "item", name = "speed-module-2", amount = global_chemical_plant_average_beacon_count * global_beacon_module_slots}
    },
    results = {
