...
local beaconed_data = {
  ...
  emission_hack                     = 1.01981
}
local beaconed_electric_furnace_1_animation_speed = beaconed_stats(beaconed_data).beaconed_animation_speed
...
beaconed_electric_furnace.crafting_speed = beaconed_stats(beaconed_data).beaconed_crafting_speed--6.2
beaconed_electric_furnace.energy_source.emissions_per_minute.pollution = beaconed_stats(beaconed_data).beaconed_emissions_per_minute
beaconed_electric_furnace.energy_source.drain = beaconed_stats(beaconed_data).beaconed_drain_string
...
if settings.startup["productivity-mode"].value == 'Modded Modules' then
  beaconed_electric_furnace.module_slots = data.raw["furnace"]["electric-furnace"].module_slots * 2
  if settings.startup["show-module-slot-row-length"].value > 0 then
    beaconed_electric_furnace.module_info_max_icons_per_row = settings.startup["show-module-slot-row-length"].value
  end
  if settings.startup["show-module-slot-rows"].value > 0 then
    beaconed_electric_furnace.module_info_max_icon_rows = settings.startup["show-module-slot-rows"].value
  end
else
  beaconed_electric_furnace.module_slots = 0
  beaconed_electric_furnace.base_productivity = beaconed_stats(beaconed_data).beaconed_base_productivity
end

if settings.startup["modded-entity-graphics"].value == "ON" then
beaconed_electric_furnace.graphics_set.animation =
{
  layers =
  {
    {
...
beaconed_electric_furnace.graphics_set.working_visualisations[2] =
{
  layers =
  {
    {
...
beaconed_electric_furnace.graphics_set.working_visualisations[3] =
{
  layers =
  {
    {
...
if settings.startup["modded-entity-graphics"].value ~= "OFF" then
  ...
  for i,layer in pairs(beaconed_electric_furnace.graphics_set.working_visualisations) do
...
data:extend({
  {
    ...
    stack_size = 50,
    localised_description = {'item-description.beaconed-electric-furnace', tostring(global_electric_furnace_beacon_count)}
  },
})
data:extend({
  {
    ...
    ingredients =
    {
      ...
      {type = "item", name = "beacon", amount = global_electric_furnace_average_beacon_count},
      {type = "item", name = "speed-module", amount = global_electric_furnace_average_beacon_count * global_beacon_module_slots}
    },
    results = {
