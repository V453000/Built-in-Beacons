...
beaconed_pumpjack_2.energy_source.emissions_per_minute.pollution = beaconed_stats(beaconed_data).beaconed_emissions_per_minute
...
beaconed_pumpjack_2.module_slots = 0

-- if settings.startup["show-module-slot-row-length"].value > 0 then
--   beaconed_pumpjack_2.module_info_max_icons_per_row = settings.startup["show-module-slot-row-length"].value
-- end
-- if settings.startup["show-module-slot-rows"].value > 0 then
--   beaconed_pumpjack_2.module_info_max_icon_rows = settings.startup["show-module-slot-rows"].value
-- end

if settings.startup["modded-entity-graphics"].value == "ON" then
  table.insert(beaconed_pumpjack_2.graphics_set.animation.north.layers,
  {
    priority = "high",
...
if settings.startup["modded-entity-graphics"].value ~= "OFF" then
  for i,layer in pairs(beaconed_pumpjack_2.graphics_set.animation.north.layers) do
...
data:extend({
  {
    ...
    stack_size = 20,
    localised_description = {'item-description.beaconed-pumpjack-2', tostring(global_pumpjack_beacon_count)}
  },
})
data:extend({
  {
    ...
    ingredients =
    {
      ...
      {type = "item", name = "speed-module-2", amount = global_pumpjack_average_beacon_count * global_beacon_module_slots}
    },
    results = {
